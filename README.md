[![npm version](https://badge.fury.io/js/cordova-plugin-media-editor.svg)](https://badge.fury.io/js/cordova-plugin-media-editor)
...
- `PERMISSION_DENIED_FIRST_TIME` means permission is denied but the app can still re-request it in-app.
- `PERMISSION_DENIED_NEED_SETTINGS` means permission cannot be resolved in the current flow (including dismissed dialog), so guide users to system settings.
...
```javascript
// parameters passed to transcodeAudio
CTYMediaEditor.transcodeAudio(
    success, // success cb
    error, // error cb
    {
        ...
        audioChannels: 2, // optional (ios only), number of audio channels, defaults to 2
        audioSampleRate: 44100, // optional (ios only), sample rate for the audio, defaults to 44100
        audioBitrate: 128000, // optional (ios only), audio bitrate for the audio in bits, defaults to 128 kilobits (128000)
        progress: function(info) {} // info will be a number from 0 to 100
    }
);
```
...
```javascript
CTYMediaEditor.createThumbnail(
    success, // success cb
    error, // error cb
    {
        fileUri: 'file-uri-here', // the path to the video on the device
...
## Android & FFmpeg
FFmpeg has been removed from android for several reasons but mainly for performance.  If you still need the old functionality that FFmpeg provided  [V1.09](https://github.com/CTY-Library/cordova-plugin-media-editor/tree/1.0.9) is the last version that will use it.
